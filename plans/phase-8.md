...
| # | Task | AC | Status |
|---|------|----|--------|
| 8.1 | Write `harness-cycle` SKILL.md that encapsulates the work → review → fix → commit loop | AC: User can invoke `/harness-cycle` and Claude executes the full loop for the active phase: picks the next `cc:TODO` task, runs the TDD loop, reviews, commits on APPROVE, or fixes and re-reviews on REQUEST_CHANGES | cc:TODO |
